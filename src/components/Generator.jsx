...
import { WORKOUTS ,SCHEMES} from '../util/Swoldier'
...
export default function Generator(props) {
  ...
  function updateMuscleGroup(muscleGroup){
  ...
  return (
      ...
      title={['It\'s',' Huge',' O\'Clock']}>
       ...
       <Header index={'01'} title={'Pick Your poison'}
        description={'select the workout you endore'} />
      ...
      {Object.keys(WORKOUTS ).map((type,typeIndex)=>{
        return (
          <button onClick={()=>{
            setMuscles([])
            setPoison(type)}}
          className={'bg-slate-950 px-4 border border-blue-400 duration-200 hover:border-blue-600 py-3 rounded-lg '
      ...
      <Header index={'02'} title={'Lock on Targets'}
        ...
        <button onClick={ToggleModel} className='relative p-3 flex items-center justify-center'>
          <p className='capitalize'>
            {muscles.length==0 ?'Select the Muscle Group':muscles.join(' ')}
            </p>
      ...
      {Object.keys(SCHEMES).map((scheme,schemeIndex)=>{
        return (
          ...
          className={'bg-slate-950 border border-blue-400 duration-200 hover:border-blue-600 py-3 px-4 rounded-lg '
          + (scheme === goal ? 'border-blue-600' : 'border-blue-400' )}
          key={schemeIndex}>
            <p className='capitalize'>
              {scheme.replaceAll('_',' ')}</p>
          </button>
        )
      })}
      </div>
